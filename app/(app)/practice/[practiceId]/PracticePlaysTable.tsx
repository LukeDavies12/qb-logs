...
import { evaluatePlayPractice } from "@/components/evaluatePlayPractice"
...
import { playExeuctionLevelsConst } from "@/types/gameTypes"
...
import { deletePlay } from "../../game/[gameId]/gameActions"
import { deleteBlock } from "./practiceActions"
import UpdatePlayFormPractice from "./UpdatePracticePlay"

interface PracticePlaysTableProps {
  blocks: (PracticeBlock & { plays: PracticePlay[] })[]
...
export default function PracticePlaysTable({
  blocks,
  playGroupings = [],
  tags = [],
  playCalls = [],
  ...
  const [selectedPlay, setSelectedPlay] = useState<PracticePlay | null>(null)
  ...
  // Derive all plays from blocks
  const plays = useMemo(() => {
    return blocks.flatMap((block) => block.plays || [])
  }, [blocks])

  // Group plays by block
  const playsByBlock = useMemo(() => {
    const grouped = new Map<number, PracticePlay[]>()
    ...
    plays.forEach((play) => {
      const blockId = play.practice_block_id
      if (blockId) {
        if (!grouped.has(blockId)) {
          grouped.set(blockId, [])
        }
        grouped.get(blockId)?.push(play)
      }
    })
  ...
  const sortedBlocks = useMemo(() => [...blocks].sort((a, b) => a.name.localeCompare(b.name)), [blocks])

  const blockOptions = useMemo(() => sortedBlocks.map((block) => block.name), [sortedBlocks])
  ...
  const filteredBlocks = useMemo(() => {
    ...
    return sortedBlocks.filter((block) => {
      if (selectedBlocks.length > 0 && !selectedBlocks.includes(block.name)) {
        return false
      }

      const blockPlays = playsByBlock.get(block.id) || []

      if (blockPlays.length === 0) {
        return selectedPlayGroupings.length === 0 && selectedPlayCalls.length === 0 && selectedTags.length === 0
      }

      const hasMatchingPlays = blockPlays.some((play) => {
  ...
  }, [sortedBlocks, selectedBlocks, selectedPlayGroupings, selectedPlayCalls, selectedTags, playsByBlock])

  const getFilteredPlays = (block: PracticeBlock) => {
    const blockPlays = playsByBlock.get(block.id) || []
  ...
  const toggleBlockDetails = (blockId: number) => {
  ...
  const handleUpdate = useCallback((practicePlay: PracticePlay) => {
    setIsModalOpen(true)
    setSelectedPlay(practicePlay)
    setModalKey((prev) => prev + 1)
  }, [])

  const handleDeletePlay = useCallback(
    (practicePlay: PracticePlay) => {
      if (window.confirm(`Are you sure you want to delete play #${practicePlay.film_number}?`)) {
        startTransition(async () => {
          try {
            const result = await deletePlay(practicePlay.id, practiceId)
            if (!result.success) {
  ...
  const handleDeleteBlock = useCallback(
    (block: PracticeBlock) => {
      if (
        window.confirm(
          `Are you sure you want to delete block "${block.name}"? This will delete all plays in this block.`,
        )
      ) {
        startTransition(async () => {
          try {
            const result = await deleteBlock(block.id, practiceId)
            if (!result.success) {
  ...
  const toggleAllBlocks = () => {
    if (expandedBlocks.size === blocks.length) {
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={expandedBlocks.size === blocks.length ? "Collapse all blocks" : "Expand all blocks"}
            >
              {expandedBlocks.size === blocks.length ? (
      ...
      <div className="h-full flex flex-col mt-2">
        <div className="flex-grow">
          <div className="space-y-2 pb-1">
            {filteredBlocks.length === 0 ? (
              <div className="p-4 text-center text-neutral-500 rounded-md">No plays match the selected filters</div>
            ) : (
              filteredBlocks.map((block) => {
                const filteredPlays = getFilteredPlays(block)
                const blockPlays = playsByBlock.get(block.id) || []
                const hasPlays = blockPlays.length > 0
                ...
                return (
                  ...
                  >
                    <div className="text-xs font-medium text-neutral-700 tracking-wider py-1 flex gap-2 items-center">
                      <span className="font-semibold uppercase">{block.name}</span>
                    ...
                    {expandedBlocks.has(block.id) && hasPlays && filteredPlays.length > 0 && (
                    ...
                    {expandedBlocks.has(block.id) && (!hasPlays || filteredPlays.length === 0) && (
                      <div className="p-4 text-center text-neutral-500 bg-neutral-50/50">
                        {!hasPlays
                          ? "No plays have been added to this block yet"
                          : "No plays match the selected filters in this block"}
                      </div>
                    )}
                  </div>
                )
        ...
        <div className="flex-shrink-0 py-2 border-t border-neutral-100 bg-white flex flex-wrap justify-between items-center px-3 sticky bottom-0">
              ...
              <>
                <div className="text-xs font-medium text-neutral-600">
                  <span className="font-semibold">Graded:</span> {(() => {
                    const filteredPlays = filteredBlocks.flatMap((block) => getFilteredPlays(block))
                    const involvedPlays = filteredPlays.filter(
                      (play) => evaluatePlayPractice(play)?.involved === true && play.result !== "Penalty",
                    )
                    const executedPlays = filteredPlays.filter(
                      (play) =>
                        evaluatePlayPractice(play)?.executed === true && evaluatePlayPractice(play)?.involved === true,
                    )
                ...
                <div className="text-xs font-medium text-neutral-600">
                  ...
                  {
                    filteredBlocks
                      .flatMap((block) => getFilteredPlays(block))
                      .filter(
                        (play) =>
                ...
                <div className="text-xs font-medium text-neutral-600">
                  <span className="font-semibold">On Schedule:</span> {(() => {
                    ...
                    const totalEligiblePlays = filteredPlays.filter((play) => {
                      const isInvolvedAndNotPenalty =
                        evaluatePlayPractice(play)?.involved === true
          ...
          <div className="text-xs font-medium text-neutral-600 rounded-full bg-neutral-100 px-2 py-0.5">
            {(() => {
              const filteredPlaysCount = filteredBlocks.reduce((total, block) => {
                return total + getFilteredPlays(block).length
              }, 0)
        ...
        >
          <div className="p-2">
            <UpdatePlayFormPractice
              ...
              tags={tags}
              blockId={selectedPlay.practice_block_id}
