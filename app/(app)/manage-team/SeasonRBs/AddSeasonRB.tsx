...
import { addSeasonRB } from "@/app/(app)/manage-team/manageTeamActions"
import ComboBox, { type ComboBoxRef } from "@/components/Combobox"

export default function AddSeasonRB({
  hasStarter
}: {
  hasStarter: boolean
}) {
  ...
  const [state, formAction, isPending] = useActionState(addSeasonRB, { error: "", success: false })
  ...
  const handleAddRB = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
