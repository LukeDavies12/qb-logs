...
import type { PlayExecutionLevel } from "@/types/gameTypes"
import { PracticePlay } from "@/types/practiceTypes"
import { Info } from "lucide-react"
import { useState } from "react"
import { evaluatePlayPractice } from "./evaluatePlayPractice"
import FullScreenModal from "./FullScreenModal"
import { PlayGroupingStatsPractice } from "./PlayGroupingAnalysisPractice"
...
function isOnSchedulePlayPractice(play: PracticePlay): boolean {
  const yardsGained = play.yards_gained || 0
  return yardsGained >= 4
}

export default function QBAnalysisPractice({ qb, plays }: { qb: { name: string; number: number }; plays: PracticePlay[] }) {
  const [isAnalysisModelOpen, setIsAnalysisModelOpen] = useState(false)
  const involvedPlaysCount = plays.filter((play) => evaluatePlayPractice(play)?.involved === true).length
  const involvedPlays = plays.filter((play) => evaluatePlayPractice(play)?.involved === true && play.result != "Penalty")
  const executedPlays = plays.filter(
    (play) => evaluatePlayPractice(play)?.executed === true && evaluatePlayPractice(play)?.involved === true,
  )
  ...
  const playsOnSchedule = plays.filter((play) => isOnSchedulePlayPractice(play)).length
  const totalEligiblePlays = plays.filter((play) => {
    const isInvolvedAndNotPenalty = evaluatePlayPractice(play)?.involved === true
  ...
  return (
    <div className="rounded-lg border bg-white p-3">
            ...
            <div className="flex items-center gap-1 group relative">
              ...
              <div className="relative">
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-2 bg-neutral-800 text-white text-xs rounded shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity z-10">
                  % of plays meeting yardage goal: 4+ yds (different on games)
      ...
      {isAnalysisModelOpen && (
        ...
        >
          <PlayGroupingStatsPractice plays={plays} />
        </FullScreenModal>
      )}
    </div>
  )
